...
let InitiateBuildResponse = ({
	HttpStatusCodes, ResponseStatus
}) => {
	// Actual response service which handles the logic for various responses
	return function Response({
		httpStatusCode, responsePayload = null, responseHeader = null
	} = {}) {
		// Get Http response codes
		let GetHttpResponseCode = (statusCodes) => {
		...
		let httpStatusCodesObject = GetHttpResponseCode(HttpStatusCodes.GetHttpCodes());
		// Sanitize http status code
		let Sanitize = (httpStatusCode) => {
			if(typeof httpStatusCode === 'string') httpStatusCode = parseInt(httpStatusCode.trim());
			return httpStatusCode;
		};
		// Get common response object
		let ResponseObject = ({
			status, statusCode, payload, header
		} = {}) => JSON.stringify({
			response: {
				status: status,
				httpCode: statusCode,
